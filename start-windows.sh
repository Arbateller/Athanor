#!/bin/bash
#
#  start-windows.sh - Start the entire BIST Tracker stack (Windows / Git Bash)
#
#  Created by Murat Şahin
#  Windows adaptation added for Git Bash compatibility
#
#  Usage:
#    ./start-windows.sh          Start all services
#    ./start-windows.sh stop     Stop all services
#
...
echo ""
echo -e "${BOLD}══════════════════════════════════════════${NC}"
echo -e "${BOLD}   BIST Tracker - Full Stack Startup (Windows)${NC}"
echo -e "${BOLD}══════════════════════════════════════════${NC}"
echo ""
...
if redis-cli ping &>/dev/null; then
  ...
  fail "Redis is not running. Please start Redis manually."
  exit 1
fi

# ── 2. Python virtual environment (Windows path) ──────
...
if [ -f "$PROJECT_DIR/venv/Scripts/activate" ]; then
  ...
  fail "Run: python -m venv venv && pip install -r requirements.txt"
  exit 1
fi
...
cd "$PROJECT_DIR"
PYTHONUTF8=1 python fetcher/fetcher.py > "$PROJECT_DIR/logs/fetcher.log" 2>&1 &
FETCHER_PID=$!
...
echo -e "  Or run: ${BOLD}./start-windows.sh stop${NC}"
echo -e "${BOLD}══════════════════════════════════════════${NC}"
echo ""
