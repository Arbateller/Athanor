...
def safe_float(val):
    """Convert to float, return 0 if None or NaN."""
    try:
        v = float(val)
    ...
    for ticker, data in cached.items():
        try:
            ...
            result.append({
                ...
                "RSI 14":        safe_float(indicators["rsi_14"]) if indicators.get("rsi_14") is not None else None,
                "MACD Line":     safe_float(indicators["macd_line"]) if indicators.get("macd_line") is not None else None,
                "MACD Signal":   safe_float(indicators["macd_signal"]) if indicators.get("macd_signal") is not None else None,
                "MACD Hist":     safe_float(indicators["macd_hist"]) if indicators.get("macd_hist") is not None else None,
